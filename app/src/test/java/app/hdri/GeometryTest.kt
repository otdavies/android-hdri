...
class GeometryTest {
    @Test fun projectionUsesPhysicalCameraAxes(){
        ...
        val q=Q.look(123.0,42.0);val p=V3(.2,.5,-.8).unit()
        assertTrue(q.inverse().rotate(q.rotate(p)).angle(p)<1e-5)
    }
    @Test fun sphericalCoordinatesWrapAndIncludeBothPoles(){
        assertTrue(Sphere.ray(0.0,250.0,1000,500).angle(Sphere.ray(1000.0,250.0,1000,500))<1e-6)
    ...
    @Test fun plannedCoverageHasNoGapsAcrossRealisticLenses(){
        for(fov in listOf(40.0,54.0,75.0,90.0)){
            ...
            for(y in 0..90)for(x in 0..180){
                ...
                assertTrue("Uncovered $x,$y at FOV $fov",targets.minOf{it.ray.angle(ray)}<fov*.455)
            }
        }
    }
    @Test fun dwellRejectsTranslationTrackingLossAndMovement(){
        val gate=SteadyGate(500_000_000);var now=1_000_000_000L
        repeat(8){gate.update(now,Q(),true,true,0.0);now+=100_000_000}
    ...
    @Test fun bracketsClampAndDeduplicateHardwareLimits(){
        ...
        assertEquals(2,limited.size);assertEquals(limited.distinct(),limited)
    }
}
