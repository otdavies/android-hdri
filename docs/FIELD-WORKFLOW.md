...
**Physical-stream routes** remain internally resolvable for older sessions: preview and JPEG outputs are pinned to a physical ID, using physical request overrides and result metadata. Android does not guarantee arbitrary physical JPEG stream combinations: see [multi-camera API](https://developer.android.com/media/camera/camera2/multi-camera). No fallback silently switches a capture to another lens. New-capture setup offers the main camera and one preferred ultrawide, prioritizing logical zoom and falling back to an independently openable wide camera. Aliases and physical-stream routes are not separate picker entries.
...
- 41 JVM checks cover radiometry, EXR blocks and truncation, camera crop/preview inverses, planner coverage and existing capture behaviour.
- Installed-APK gate requires 32 tests, including GPU diffuse/exposure checks, GPU field-of-view changes, ground-fill boundaries and pole continuity, cancelled and successful master migration, orphan cleanup, UI controls, and complete sample processing with the EXR master.
...
## Native preview orientation correction

Camera2's SurfaceTexture transform includes the rotation into the device's natural orientation, plus the GL vertical flip, buffer crop and texel inset. The previous native path applied that rotation after already transforming display coordinates into raw sensor coordinates for guidance. That rotated the preview twice while the markers and recorded JPEG pose used only the sensor rotation.

The native sampler now factors out the texture matrix's orthogonal orientation about its sampling centre, retaining its crop and scale. Both preview corners and target markers use the same sensor-to-display mapping. Sensor/JPEG pose conventions and the ARCore main-camera preview stay as before. The texture transform is read again for each frame.

An external-OES GPU regression draws a raw sensor chart into a real SurfaceTexture, adds simulated Camera2 buffer-rotation metadata, and uses the production native sampler. It checks independently specified corner colours and circle aspect ratio for 64 combinations of sensor, display and producer rotation. A portrait readback image and the per-case GPU results are included in CI verification. This closes the gap in earlier inverse-coordinate tests, which could pass even when the rendered texture rotated a second time.

Primary implementation references: [AndroidX OpenGL preview renderer](https://android.googlesource.com/platform/frameworks/support/+/refs/heads/androidx-main/camera/integration-tests/coretestapp/src/main/java/androidx/camera/integration/core/OpenGLRenderer.java), [Camera2 preview orientation](https://developer.android.com/media/camera/camera2/camera-preview), and [SurfaceTexture transforms](https://developer.android.com/reference/android/graphics/SurfaceTexture#getTransformMatrix(float[])). These tests exercise rendering; Pixel 8 camera output itself still needs a hardware check.

### Published Preview 21

[Release and APK](https://github.com/otdavies/android-hdri/releases/tag/v0.1.0-preview.21), from source `dc2e887eb8245713977ba7ad1bc9f6567a40fd12`. [GitHub run 34075117194](https://github.com/otdavies/android-hdri/actions/runs/34075117194) passed build, device and publication. All 41 JVM tests passed; lint reported zero errors and eight advisory warnings. All 32 installed-APK tests passed in 67.895 seconds. Independent OpenEXR 3.3.3 decoding checks passed.

The native external-texture GPU test passed all 64 sensor/display/producer rotation combinations. Every corner matched the independently specified chart orientation. Horizontal/vertical circle radii differed by at most three raster pixels; the portrait sensor-90/display-0/producer-90 example measured 90 and 91 pixels. Its GPU readback was visually reviewed: blue/red above yellow/green, with a round white centre, matching the raw chart rotated clockwise once. The fixture models Camera2 buffer orientation but does not replace a Pixel 8 camera hardware check.

Published APK: 124,004,089 bytes; SHA-256 `0907ff6b2acf232c975f996aa69d2e2bda8faba819f439dea644960225ac0895`. The checked device-verification archive has SHA-256 `101886287a868fd66019279f5ea668471d1abbc04afb300d41bc83d685b188b2`.
